...
# 8-5  本来想优化一下时间检测的结构，想想也没优化多少，算了
...
class WeiboSpiderSpider(scrapy.Spider):
    ...
    test_list = []
    pre_save_file_path = "./file" + "/" + get_key_word() + "/prefile/simple_wb_info.json"
    if os.path.exists(pre_save_file_path):
        saved_wb = read_json(pre_save_file_path)
        saved_keys = list(saved_wb.keys())
    else:
        save_keys = []
    ...
    def parse_homepage_part3(self, response):
        meta = response.meta
        ...
        divs = self.filter_like_div(divs)


    def filter_like_div(self,divs):
        real_div_list = []
        for div in divs:
    ...
    def parse_weibo_from_div(self, div, get_all_comment, get_all_r_comment, t_bid, check_time):
        ...
        # 如果有折叠的部分
        if is_content_long:
            content_parse = self.rquests_get_weibo_div(self.session, weibo_url)
        ...
        if item_eles and is_original:
            ...
            # 存在图片ele
            if img_info_list:
                img_list = [self.get_img_list(img_info) for img_info in img_info_list]
            else:
                img_list = []
    ...
    def parse_comment_from_div(self, h_comment_div, comment_type, superior_id, ):
        """
        :param h_comment_div: div，我也不记得我为什么要加h_了
    ...
    def get_img_list(self, img_info):
        # 解析图片列表
