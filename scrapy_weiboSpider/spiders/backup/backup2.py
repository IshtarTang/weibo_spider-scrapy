...
class WeiboSpiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        check_url = "https://weibo.com/{}?page=1&is_all=1".format(user_id)
        # 测试与获取用户标识
        yield scrapy.Request(check_url, callback=self.check_and_get_ident, cookies=self.cookies,
                             meta={"count": 0, "user_id": user_id, "url": check_url}, errback=self.deal_err,
                             dont_filter=True)
        # 第一部分和后两部分的链接不同
        ...
        for page in range(1, self.page_num + 1):
            ...
            if self.config["personal_homepage_config"]["time_range"]["enable"]:
                # 检查这一页是否需要进行请求
                self.later_flag = 1
                page_in_timerange = self.n_is_page_in_timerange(user_id, page)
                # 晚了，跳过该页
            ...
            # url2和url3带时间戳，两次重复启动的时候链接会变，但1不带时间戳，需要加上dont_filter=True，否则会被过滤掉
    ...
    def parse_home_page(self, response):
        ...
        if response.meta["part"] == 1:
            divs = self.n_get_div_from_part1_response(response)
            # # 循环完所有的script后如果没有找到html代码就重新亲请求
            # 没找到divs就重新请求
            if not divs:
                meta["count"] += 1
        ...
        if len(real_div_list) < 15:
            # 循环次数大于4次,说明只有这么多条，则不重新获取，继续解析。max_div用于确认当前div数量是能获取到的最大div。
        ...
        if self.config["personal_homepage_config"]["time_range"]["enable"] \
                and not self.is_divs_in_time_range(
        ...
        for div in real_div_list:
            ...
            r_weibo_and_requests = self.n_parse_weibo_from_div(div, get_all_comment, get_all_r_comment, "")
    ...
    def get_root_comment(self, response):
        ...
        if not root_comm_divs:
            if meta["count"] > 5:
                return
            meta["count"] += 1
            yield Request(response.url, cookies=self.cookies, meta=meta, dont_filter=True, errback=self.deal_err)
        ...
        for root_comm_div in root_comm_divs:
            # 解析root评论，塞进pipeline
            comm_item = self.n_parse_comment_from_div(root_comm_div, "root", meta["superior_id"])
            yield comm_item
            ...
            if child_comment_divs:
                ...
                for child_comment_div in child_comment_divs:
                    comm_item = self.n_parse_comment_from_div(child_comment_div, "child", root_comm_id)
                    yield comm_item
                    # 是否有更多子评论
    ...
    def parse_weibo_page(self, response):
        ...
        if weibo_parse == None:
            ...
            if meta["count"] < 6:
                meta["count"] += 1
                yield Request(response.url, callback=self.parse_weibo_page, meta=meta, cookies=self.cookies,
                              errback=self.deal_err)
                return
            # # 是转发微博的源微博
        ...
        request_and_item = self.n_parse_weibo_from_div(weibo_div, meta["get_all_comment"], meta["get_all_r_comment"],
                                                       meta["t_bid"])
        for r_or_i in request_and_item:
            yield r_or_i

    def n_parse_weibo_from_div(self, div, get_all_comment, get_all_r_comment, t_bid):
        """
        解析微博div
        这个方法不负责检查div是否是一个正常的有内容的div，如果解析不出内容会返回None，请在调用它的地方处理

        :param div: //div[@tbinfo]
        ...
        if self.config["mode"] == 1 and self.config["personal_homepage_config"]["time_range"]["enable"]:
            if not self.is_time_in_range(public_timestamp):
                ...
                logging.info(
                    "当前微博[{}]{} 时间不在要求范围内[{}-{}]，被过滤掉"
                        .format(public_timestamp, content[:10],
        ...
        # 如果有折叠的部分
        if is_content_long:
            content_parse = self.n_get_weibo_div(self.session, weibo_url)
        ...
        if r_href and "源微博已不可见" not in remark:
            ...
            # 源微博的get_all_r_comment不会被用到，写啥都无所谓
            meta["get_all_comment"] = get_all_r_comment
            meta["get_all_r_comment"] = 0
            yield Request(r_url, callback=self.parse_weibo_page, cookies=self.cookies, meta=meta,
                          errback=self.deal_err)
    ...
    def n_parse_comment_from_div(self, h_comment_div, comment_type, superior_id, ):
    ...
    def is_divs_in_time_range(self, divs, check_early, check_later):
        """
        ...
        if start_time and check_early:
            ...
            if self.is_a_early_than_b(wb_stop_time, start_time, False):
                result = False
        ...
        if stop_time and check_later:
            ...
            if self.is_a_early_than_b(stop_time, wb_start_time, True):
                result = False
    ...
    def is_time_in_range(self, time1):
        """
        包早不包晚
        :param time: 毫秒时间戳(13位)或 "%Y-%m-%d %H:%M"格式的字符串
        :return:
        """
        ...
        if start_time and not self.is_a_early_than_b(start_time, time1, True):
            return False
        if stop_time and not self.is_a_early_than_b(time1, stop_time, False):
            return False
        return True

    def is_a_early_than_b(self, a, b, can_equal):
    ...
    def n_get_weibo_div(self, session, url):
    ...
    def n_get_div_from_part1_response(self, response):
        """
    ...
    def n_get_img_list(self, img_info):
        # 解析图片列表
    ...
    def write_test(self, file, filename="test_code.html", coding="utf-8"):
        path = "./test_file"
        open(path + "/" + filename, "w", encoding=coding).write(file)

    def n_is_page_in_timerange(self, user_id, page):
        result = "fine"
        first_part_url_base = "https://weibo.com/u/{}?page={}&is_all=1"
        sub_part_url_base = "https://weibo.com/p/aj/v6/mblog/mbloglist?ajwvr=6&domain=100505&visible=0&is_all=1" \
                            "&profile_ftype=1&page={}&pagebar={}&pl_name=Pl_Official_MyProfileFeed__20" \
        ...
        if self.later_flag:
            ...
            # 整页时间晚于规定时间范围，跳过该页
            while True:
                p3_response = self.session.get(url3)
                self.write_test(p3_response.text, "p3_test.html")
                try:
            ...
            if not self.is_divs_in_time_range(p3_divs, check_early=False, check_later=True):
                logging.info("page {} 时间晚于限定时间，跳过")
                result = "later"
            else:
        ...
        if result == "fine":
            while True:
                p1_response = self.session.get(url1)
                p1_divs = self.n_get_div_from_part1_response(p1_response)
                if p1_divs:
                    break
            if not self.is_divs_in_time_range(p1_divs, check_early=True, check_later=False):
                logging.info("page {} 时间早于规定时间范围")
                result = "early"
