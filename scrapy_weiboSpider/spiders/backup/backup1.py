...
class WeiboSpiderSpider(scrapy.Spider):
    ...
    def parse_home_page(self, response):
        ...
        if len(real_div_list) < 15:
            ...
            if meta["count"] > 4 and len(real_div_list) == meta["max_div"]:
                ...
                return
        # 该divs不在时间范围内，不做保存
        if not self.is_divs_in_time_range(real_div_list, True, True):
            print("不存")
            return
    ...
    def parse_weibo_page(self, response):
        ...
        :return:
        """
        logging.info("接收到 {}".format(response.url))
    ...
    def n_parse_weibo_from_div(self, div, get_all_comment, get_all_r_comment, t_bid):
        ...
        if r_href and "源微博已不可见" not in remark:
            ...
            logging.info("{} 准备递送".format(r_url))
    ...
    def n_parse_comment_from_div(self, h_comment_div, comment_type, superior_id, ):
        ...
        if comment_link_ele:
            ...
            try:
                real_comment_link = self.session.get(comment_link).url
            except:
                pass
    ...
    def is_time_in_range(self, time1):
        ...
        start_time = self.config["personal_homepage_config"]["time_range"]["start_time"]
        stop_time = self.config["personal_homepage_config"]["time_range"]["stop_time"]
        if not self.is_a_early_than_b(start_time, time1, True):
            return False
        if not self.is_a_early_than_b(time1, stop_time, False):
            return False
        return True
    ...
    def n_get_div_from_part1_response(self, response):
        ...
        if isinstance(response, scrapy.http.HtmlResponse):
            ...
            for script_ele in script_eles:
                script = etree.tostring(script_ele, encoding="utf-8")
